...
# Detect rom folder and handle fallback for modern versions
patchver="$sourcever"
if [[ ! -d "$romsdir/$patchver/$romtype" ]]; then
    echo "No direct profile found for $romtype on Android $sourcever. Finding latest base..."
    for v in 12 11 10 9; do
        if [[ -d "$romsdir/$v/$romtype" ]]; then
            patchver="$v"
            echo "Using Android $patchver profile as fallback base"
            break
        fi
    done
fi
...
# Debloat
$romsdir/$patchver/$romtype/debloat.sh "$systemdir/system" 2>/dev/null
$romsdir/$patchver/$romtype/$romtypename/debloat.sh "$systemdir/system" 2>/dev/null

# Resign to AOSP keys (Skipped automatically on Android 12+ to prevent signature failures)
is_modern=0
if [ "$sourcever" -ge 12 ] 2>/dev/null; then
    is_modern=1
fi

if [ $is_modern -eq 0 ]; then
    if [[ ! -e $romsdir/$patchver/$romtype/$romtypename/DONTRESIGN ]] && [[ ! -e $romsdir/$patchver/$romtype/DONTRESIGN ]]; then
        echo "Resigning to AOSP keys"
        ispython2=`python -c 'import sys; print("%i" % (sys.hexversion<0x03000000))' 2>/dev/null || echo 0`
        if [ $ispython2 -eq 0 ]; then
            python2=python2
        else
            python2=python
        fi
        $python2 $toolsdir/ROM_resigner/resign.py "$systemdir/system" $toolsdir/ROM_resigner/AOSP_security > $tempdir/resign.log 2>/dev/null || true
        $prebuiltdir/resigned/make.sh "$systemdir/system" 2>/dev/null || true
    fi
else
    echo "Modern Android build ($sourcever) detected: skipping legacy APK resignation"
fi
...
# Apply VNDK fix
if [[ -f "$prebuiltdir/vendor_vndk/make$patchver.sh" ]]; then
    $prebuiltdir/vendor_vndk/make$patchver.sh "$systemdir/system" 2>/dev/null
elif [[ -f "$prebuiltdir/vendor_vndk/make11.sh" ]]; then
    $prebuiltdir/vendor_vndk/make11.sh "$systemdir/system" 2>/dev/null
fi

# Apply prebuilts
if [[ -d "$prebuiltdir/$patchver" ]]; then
    $prebuiltdir/$patchver/make.sh "$systemdir/system" "$romsdir/$patchver/$romtype" 2>/dev/null
    $prebuiltdir/$patchver/makeroot.sh "$systemdir" "$romsdir/$patchver/$romtype" 2>/dev/null
elif [[ -d "$prebuiltdir/11" ]]; then
    $prebuiltdir/11/make.sh "$systemdir/system" "$romsdir/$patchver/$romtype" 2>/dev/null
    $prebuiltdir/11/makeroot.sh "$systemdir" "$romsdir/$patchver/$romtype" 2>/dev/null
fi

$prebuiltdir/common/make.sh "$systemdir/system" "$romsdir/$patchver/$romtype" 2>/dev/null
$romsdir/$patchver/$romtype/make.sh "$systemdir/system" 2>/dev/null
$romsdir/$patchver/$romtype/makeroot.sh "$systemdir" 2>/dev/null
if [ ! "$romtype" == "$romtypename" ]; then
    $romsdir/$patchver/$romtype/$romtypename/make.sh "$systemdir/system" 2>/dev/null
    $romsdir/$patchver/$romtype/$romtypename/makeroot.sh "$systemdir" 2>/dev/null
fi
if [ "$outputtype" == "Aonly" ] && [ ! "$romtype" == "$romtypename" ]; then
    $romsdir/$patchver/$romtype/$romtypename/makeA.sh "$systemdir/system" 2>/dev/null
fi
if [ "$outputtype" == "Aonly" ]; then
    if [[ -f "$prebuiltdir/$patchver/makeA.sh" ]]; then
        $prebuiltdir/$patchver/makeA.sh "$systemdir/system" 2>/dev/null
    elif [[ -f "$prebuiltdir/11/makeA.sh" ]]; then
        $prebuiltdir/11/makeA.sh "$systemdir/system" 2>/dev/null
    fi
    $romsdir/$patchver/$romtype/makeA.sh "$systemdir/system" 2>/dev/null
fi
